...
namespace JP.Base.DAL.ADO.EntityMapper
{
    public interface IEntityDbMapper<TEntity> where TEntity : class
    {
        ...
        CommandData GetSelectCommand<TSource, TProperty>(Func<TEntity, string> filter = null, Expression<Func<TSource, TProperty>> orderBy = null, ListSortDirection order = ListSortDirection.Ascending, int pageStart = 0, int pageEnd = 0);
